...
func (_q *LogsQuery) QueryLog() *URLQuery {
	query := (&URLClient{config: _q.config}).Query()
...
func (_q *LogsQuery) WithLog(opts ...func(*URLQuery)) *LogsQuery {
	query := (&URLClient{config: _q.config}).Query()
...
func (_q *LogsQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Logs, error) {
	...
	if query := _q.withLog; query != nil {
		if err := _q.loadLog(ctx, query, nodes, nil,
			func(n *Logs, e *Url) { n.Edges.Log = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LogsQuery) loadLog(ctx context.Context, query *URLQuery, nodes []*Logs, init func(*Logs), assign func(*Logs, *Url)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Logs)
	for i := range nodes {
		if nodes[i].url_log_url == nil {
			continue
		}
		fk := *nodes[i].url_log_url
