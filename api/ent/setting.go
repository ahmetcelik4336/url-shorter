...
type Setting struct {
	...
	// SettingContent holds the value of the "setting_content" field.
	SettingContent string `json:"setting_content,omitempty"`
...
func (*Setting) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case setting.FieldSettingsKey, setting.FieldSettingContent:
...
func (_m *Setting) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case setting.FieldSettingContent:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field setting_content", values[i])
			} else if value.Valid {
				_m.SettingContent = value.String
			}
		default:
...
func (_m *Setting) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("setting_content=")
	builder.WriteString(_m.SettingContent)
	builder.WriteByte(')')
