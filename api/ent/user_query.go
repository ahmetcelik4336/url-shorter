...
func (_q *UserQuery) QueryURL() *URLQuery {
...
func (_q *UserQuery) WithURL(opts ...func(*URLQuery)) *UserQuery {
	query := (&URLClient{config: _q.config}).Query()
...
func (_q *UserQuery) loadURL(ctx context.Context, query *URLQuery, nodes []*User, init func(*User), assign func(*User, *Url)) error {
	...
	query.Where(predicate.Url(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.URLColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_url
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_url" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
